...
@Command(name="anime", group="anime")
public class AnimeCommand {
    ...
    public void run(JDA bot, GuildMessageReceivedEvent event, String[] args) {
        ...
        try {
            ...
            event.getChannel().sendMessage("Please specify an anime for me to search for.").queue();
            return;
        }

        try {
            CloseableHttpResponse response = client.execute(
                    new HttpGet(String.format("https://api.jikan.moe/search/anime/%s/1", term.toString().replace(" ", "%20"))));
            ...
            event.getChannel().sendMessage("No anime were found.").queue();
        }
