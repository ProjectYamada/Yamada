...
public class TickleCommand {
    public void run(JDA bot, GuildMessageReceivedEvent event, String[] args) {
        ...
        try {
            CloseableHttpResponse response = client.execute(new HttpGet("https://nekos.life/api/v2/img/tickle"));
            String responseBody = EntityUtils.toString(response.getEntity()).replace("\n", "");
            JSONObject json = new JSONObject(responseBody);
            embed.setTitle("Tickle!", json.getString("url"));
            embed.setDescription(String.format("%s tickled %s. Awww...", event.getAuthor().getName(), member.getName()));
            embed.setImage(json.getString("url"));
        } catch (Exception e) {
            // EMPTY WOO HOO
        }
        embed.setFooter(String.format("Such weirdness %s", event.getAuthor().getName()), event.getAuthor().getAvatarUrl());
